...
function updateTimerList()
{
	...
	if(parent.party_list != null && character.party != null)
	{
		...
		{
			var partyMember = parent.party[id];
			
			var dps = getDPS(id);
			if (id == character.name) {
				listString = listString + '<tr align="left"><td align="center" style="color: yellow; background-color:#016107">' + id + '</td><td width="80px">' + dps + '</td></tr>';
			} else if (partyMember.type != "merchant") {
				listString = listString + '<tr align="left"><td align="center">' + id + '</td><td width="80px">' + dps + '</td></tr>';
			}
		}
	}
	else
	{
		...
		listString = listString + '<tr align="left"><td align="center">' + character.name + '</td><td width="80px">' + dps + '</td></tr>';
	}
	
	if(parent.party_list != null && character.party != null)
	{
		var dps = getTotalDPS();
		listString = listString + '<tr align="left"><td>' + "Total" + '</td><td width="80px">' + dps + '</td></tr>';
	}
...
function getDPS(partyMember)
{
	...
	if(entry != null)
	{
		...
		dps = parseFloat(Math.round((entry.sumDamage/(elapsed/1000)) * 100) / 100).toFixed(0);
	}
	return dps;
}
...
{	
	...
	if(minStart != null)
	{
		...
		dps = parseFloat(Math.round((sumDamage/(elapsed/1000)) * 100) / 100).toFixed(0);
	}
