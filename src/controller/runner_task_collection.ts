...
export function getChangeSpotTask(
    ...
    config: { attack?: Strategy<PingCompensatedCharacter>; move?: Strategy<PingCompensatedCharacter> }
): RunnerTask {
    if (!config) return;
...
export function getCheckBossesTask(
...
export function getInteractWithQuestNpcTask(runner: CharacterRunner<PingCompensatedCharacter>, action: string): RunnerTask | undefined {
    ...
    let interactTask = async (runner: CharacterRunner<PingCompensatedCharacter>, signal: AbortSignal) => {
        ...
        if (action == "get") {
            await runner.bot.getMonsterHuntQuest();
        } else if (action == "complete") {
            await runner.bot.finishMonsterHuntQuest();
        }
    };
...
async function checkTargetsForMs(
    bot: PingCompensatedCharacter,
    targets: MonsterName[],
    waitForMs: number,
    signal: AbortSignal
