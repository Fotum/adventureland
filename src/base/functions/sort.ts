...
export function sortPriority(bot: Character, types?: MonsterName[]): (a: Entity, b: Entity) => boolean {
    return (a: Entity, b: Entity): boolean => {
        ...
        if (aoe) {
            ...
            if (a_evasion < b_evasion)
                return true;
            else if (a_evasion > b_evasion)
                return false;
        }
        ...
        if (!a_willDie && b_willDie)
            return true;
        else if (a_willDie && !b_willDie)
            return false;

        // Will burn to death -> lower priority
        const a_willBurn = a.willBurnToDeath();
        const b_willBurn = b.willBurnToDeath();
        if (!a_willBurn && b_willBurn)
            return true;
        else if (a_willBurn && !b_willBurn)
            return false;
        ...
        if (!a_couldDie && b_couldDie)
            return true;
        else if (a_couldDie && !b_couldDie)
            return false;
...
export function sortTypeThenClosest(to: Character, types: MonsterName[]): (a: IPosition & { type: MonsterName; }, b: IPosition & { type: MonsterName; }) => number {
    return (a: IPosition & { type: MonsterName; }, b: IPosition & { type: MonsterName; }) => {
        ...
        const a_index: number = types.indexOf(a.type);
        const b_index: number = types.indexOf(b.type);
        if (a_index < b_index)
            return -1;
